# TICKET-021 - Virtual mic audio sink routing

Phase: 1 - ML PoC
Epic: Output
Estimate: M
Depends on: TICKET-019, TICKET-020
Status: Not started

## Goal

Ship `sabi.output.virtual_mic.VirtualMicSink` - a thin audio writer that consumes `TTSFrame`s from TICKET-020 and plays them into the VB-Cable virtual input device resolved in TICKET-019. Zoom / Teams / Meet, configured to use `CABLE Output` as their microphone, then hear the synthesized voice. Resamples if needed (Kokoro at 24 kHz, VB-Cable commonly 48 kHz). Exposes a mute bit TICKET-026 can flip in under 1 ms without touching the TTS engine.

## System dependencies

- VB-Cable installed and detected per TICKET-019.
...
- Define `VirtualMicConfig`: resolved input device index (from TICKET-019), output sample rate (auto-detected from the device, typically 48000), block_ms 20, latency preference `"low"`, channels 1, start_muted False.
- Implement `VirtualMicSink`:
  ...
  - `.play_stream(tts_stream: TTSStream) -> PlayResult`:
    ...
    - Captures wall-clock from first-frame-received to first-frame-written-to-device, separate from the TTS TTFB that TICKET-020 measures.
  ...
  - `.drop_queued()`: clears the internal queue without stopping the stream - used by TICKET-026 if the user hits mute mid-utterance to prevent buffered audio from leaking out on unmute.
...
- [ ] Without VB-Cable installed, `VirtualMicSink.__enter__` raises `VirtualMicNotInstalledError` (reused from TICKET-019) with a remediation message pointing to `docs/INSTALL-VBCABLE.md`.
...
- Roadmap Flow 2 UX note (project_roadmap.md line 140) - "Mute/unmute toggle must be instant - never block the meeting mic behind the pipeline." The `mute()` contract here is what TICKET-026 depends on.
